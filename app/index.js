...
if (searchParams.has('campain')) {
  ...
  param = $('#campain option:selected').val();
}

console.log(param);

$('#campain').on('change', '', function(e) {
  window.location = '?campain=' + this.value;
});
...
map.on('load', function() {
  ...
  $.ajax({
    ...
    error: function() {
      UIkit.sticky('.sticky', { offset: 50, top: 200 });

      // Functional components should omit the 'element' argument.
      UIkit.notification('Keine Daten für Aktionszeitraum ' + param + ' gefunden!', 'danger');
      console.log('Keine Daten für Aktionszeitraum ' + param + ' gefunden!')
      $('#mySpinner').hide();
    }
  });
